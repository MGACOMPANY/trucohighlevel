// index.js - Bot de Truco High Level
...
const detectarDuelo = (msg) => {
    ...
    const entrada = parseInt(entradaRaw.replace(/\./g, '')) || 0;
    const premio  = parseInt(premioRaw.replace(/\./g, '')) || 0;
...
client.on('message_create', async (msg) => {
  ...
  if (body === 'resumen ahora') {
    ...
    await msg.reply('📊 Resumen diario generado manualmente.');
    return;
  }
  ...
  if (match) {
    ...
    try {
      ...
      await chat.sendMessage(mensaje, { mentions: menciones.map(m => m.replace('@', '') + '@c.us') });
  ...
  if (msg.type === 'sticker' && msg.hasQuotedMsg) {
    ...
    const data = detectarDuelo(quoted.body);
    ...
    const yaRegistrado = existentes.some(r =>
      r[0] === data[0] && r[1] === String(data[1]) && r[2] === String(data[2]) && r[3] === data[3]
    );
  ...
  if (/^(gano|gana)\s+@/i.test(body) && msg.hasQuotedMsg) {
    ...
    await client.sendMessage(`${nroGanador}@c.us`,
      `🏆 ¡Felicidades ${ganador}!\nTu nuevo saldo es: $${saldoGanador.toLocaleString('es-AR')}.\nTRUCO HIGHLEVEL 🔝`);

    await client.sendMessage(`${nroPerdedor}@c.us`,
      saldoPerdedor === 0
  ...
  if (/^(carga|cargo|retira|retiro)\s+\d+[kK.]*/.test(body) && msg.fromMe) {
    ...
    const jugador = '@' + msg.to.replace('@c.us', '');
    const operador = '@' + msg.from.replace('@c.us', '');
    const now = new Date().toLocaleString('es-AR');
